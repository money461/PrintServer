...
public class JdbcTemplateSupport extends JdbcTemplate {
	...
	private final Logger log  = LoggerFactory.getLogger(this.getClass());
		...
		public <T> PageBean<T> queryForPage(String sql, Page<T> pagination, Object[] values,RowMapper<T> rowMapper) {
			        ...
			        String countSql="select count(1) as count from ("+sql+") temp";
			        log.info("countSql {}",countSql);
			        List<Integer> countList= super.query(countSql, values, new RowMapper<Integer>() {
			            public Integer mapRow(ResultSet resultSet, int i) throws SQLException {
			                return new Integer(resultSet.getInt("count"));
			            }
			        });
			        ...
			        sql+=parseLimit(result); //拼接条件
			        log.info("queryLimitSql {}",sql);
		
			        List<T> data= super.query(sql,values,rowMapper);
			        result.setList(data);
			        return result;
	 }
